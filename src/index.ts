...
app.post(
  "/api/gmail/new",
  async (req: Request<unknown, unknown, GmailNewBodyItem[]>, res: Response) => {
    try {
      const first =
        Array.isArray(req.body) && req.body.length > 0 ? req.body[0] : null;
      if (
        !first ||
        (first.type !== "etempmail" &&
          first.type !== "imailedu" &&
          first.type !== "edumailfree")
      ) {
        res.status(400).json({
          status: "error",
          message:
            'Invalid body. Expected: [{ "type": "etempmail" | "imailedu" | "edumailfree" }]',
        });
        return;
      }
      ...
      const result = await createImailEduAddress(
        browserInstance,
        excludeKeywords,
      );
      res.json({ status: "ok", result });
...
const READ_INVALID_BODY_MSG =
  'Invalid body. Expected: [{ "type": "etempmail" | "imailedu" | "edumailfree", "email": "<your_email>" }] or { "type": "...", "email": "..." }';

app.post(
  "/api/gmail/read",
  async (
    req: Request<unknown, unknown, GmailReadBodyItem[] | GmailReadBodyItem>,
    res: Response,
  ) => {
    try {
      const raw = req.body;
      const first: GmailReadBodyItem | null =
      ...
      if (first.type === "etempmail") {
        const result = await readEtempmailInbox(
      ...
      if (first.type === "edumailfree") {
        const result = await readEdumailfreeInbox(
          null as unknown as Browser,
          first.email,
        );
        res.json({ status: "ok", result });
      ...
      const result = await readImailEduInbox(
        null as unknown as Browser,
        first.email,
      );
      res.json({ status: "ok", result });
