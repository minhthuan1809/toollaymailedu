module.exports = function hookBasicMethods(page) {
  ...
  page.repeatFunctionByAmount = async function repeatFunctionByAmount(
    functionCallback,
    numberOfTries,
    errorMessage
  ) {
    if (numberOfTries < 1)
      throw new Error("Number of tries cannot be less than 1");
    let errorRep;
  ...
  page.executeFunctionWithTimeout = function executeFunctionWithTimeout(
    functionCallback,
    timeout,
    customMessage
  ) {
    ...
    return Promise.race([functionCallback(), timeoutPromise]).finally(() =>
      clearTimeout(timer)
    );
  };
