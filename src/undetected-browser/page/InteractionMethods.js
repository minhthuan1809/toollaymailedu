...
module.exports = function hookInteractionMethods(page) {
  ...
  page.simulateMouseClick = async function simulateMouseClick(selector) {
    ...
    await page.cursor.click(selector, {
      hesitate: page.getRandomInt(200, 800),
      waitForClick: 0,
      moveDelay: 0,
    });
  };
  ...
  page.simulateTyping = async function simulateTyping(selector, text) {
    ...
    if (!element) {
      throw new Error(
        `Element with selector ${selector} not found for typing....`
      );
    }
  ...
  page.clickElementWithInnerText = async function clickElementWithInnerText(
    selectorElement,
    innerText
  ) {
    const element = await page.getElementWithInnerText(
      selectorElement,
      innerText
    );
    if (element) await page.simulateMouseClick(element);
  };
  page.clickElementWithInnerHTML = async function clickElementWithInnerHTML(
    selectorElement,
    innerHTML
  ) {
    const element = await page.getElementWithInnerHTML(
      selectorElement,
      innerHTML
    );
    if (element) await page.simulateMouseClick(element);
